...
parser.add_argument('--traj', type=str,  help='path to the trajectory')  # /mnt/user/datasets/replica/room_0/traj_w_c.txt
parser.add_argument('--output_mesh', type=str,  help='path to the output mesh')
args = parser.parse_args()

args.input_mesh = '/mnt/user/so_nice_slam_1112/checkpoints/replica/room_0/room_0_mesh.ply'
args.traj = '/mnt/user/datasets/replica/room_0/traj_w_c.txt'
args.output_mesh = '/mnt/user/so_nice_slam_1112/checkpoints/replica/room_0/room_0_cull_mesh.ply'
...
for i in tqdm(range(0, n_imgs, 1)):
    ...
    edge = 0
    mask = (0 <= -z[:, 0, 0].cpu().numpy()) & (uv[:, 0] < W - edge) & (uv[:, 0] > edge) & (uv[:, 1] < H-edge) & (uv[:, 1] > edge)
    whole_mask &= ~mask
pc = mesh.vertices
faces = mesh.faces
